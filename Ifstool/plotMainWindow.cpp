...
void MainWindow::plotData(QVector<double> r, QVector<double> g, QVector<double> b, PLOT_TYPE type) {
    customPlot->addGraph();
    ...
    if(HIST == type) {
        customPlot->xAxis->setLabel("Histogram Bins");
        customPlot->yAxis->setLabel("Repititions");
    } else if(RGB == type){
        customPlot->xAxis->setLabel("Data points");
        customPlot->yAxis->setLabel("RGB");
    }

    // pass data points to graphs:
    customPlot->graph(0)->setData(x, r);
    customPlot->graph(1)->setData(x, g);
    customPlot->graph(2)->setData(x, b);
...
void MainWindow::setupScatterStyleDemo(QCustomPlot *customPlot) {
    ...
    for (int i = 0; i < shapes.size(); ++i) {
        ...
        customPlot->graph()->setName(
                QCPScatterStyle::staticMetaObject.enumerator(
...
void MainWindow::setupAdvancedAxesDemo(QCustomPlot *customPlot) {
    ...
    // create and configure plottables:
    QCPGraph *mainGraph1 = customPlot->addGraph(wideAxisRect->axis(QCPAxis::atBottom),
    ...
    QCPGraph *mainGraph2 = customPlot->addGraph(wideAxisRect->axis(QCPAxis::atBottom),
    ...
    if (key - lastFpsKey > 2) // average fps over 2 seconds
        ...
        ui->statusBar->showMessage(
                QString("%1 FPS, Total Data points: %2").arg(frameCount / (key - lastFpsKey), 0, 'f', 0).arg(
                        ui->customPlot->graph(0)->data()->count() + ui->customPlot->graph(1)->data()->count()), 0);
        lastFpsKey = key;
        frameCount = 0;
    }
}
...
void MainWindow::bracketDataSlot() {
    ...
    if (key - lastFpsKey > 2) // average fps over 2 seconds
        ...
        ui->statusBar->showMessage(
                QString("%1 FPS, Total Data points: %2").arg(frameCount / (key - lastFpsKey), 0, 'f', 0).arg(
                        ui->customPlot->graph(0)->data()->count()), 0);
        lastFpsKey = key;
        frameCount = 0;
    }
}
